...
class MyIndexedDB {
  constructor(options) {
    let { name = 'MyIndexedDB', version = 1, stores, use } = options || {}

    if (!stores || !Array.isArray(stores) || !stores.length) {
      stores = [
        {
          name: use || 'MyIndexedDB',
          isKeyValue: true,
        },
      ]
    }
  ...
  transaction(writable = false) {
    ...
    const request = (runtime) => () => {
      return this.db().then((db) => {
        ...
        tx.onerror = (e) => {
          runtime.reject(tools.modifyError(e))
          runtime.complete()
        }
        tx.onabort = (e) => {
        ...
        setTimeout(() => {
          if (runtime && runtime.status) {
            runtime.reject(new Error('[MyIndexedDB]: transaction timeout'))
            runtime.complete()
          }
        }, 10 * 1000)
        return tx
      ...
      this.cursor({
        ...
        onTouch: (cursor, tx, owner) => {
          if (cursor) {
            ...
            let targetValue = tools.parse(targetObj, keyPath)
            if (compare === '!=') {
  ...
  select(conditions) {
    ...
    let determine = function (obj) {
      ...
      for (let i = 0, len = and_conditions.length; i < len; i++) {
        ...
        let current = tools.parse(obj, keyPath)
  ...
  remove(obj) {
    return this.keyPath().then((keyPah) => {
      let key = tools.parse(obj, keyPah)
      if (key === undefined) {
        throw new Error(`[MyIndexedDB]: your passed object to remove() does not contain keyPath '${keyPah}'.`)
      }
